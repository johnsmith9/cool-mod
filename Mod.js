...
const mwWeight = 1;
let mwElem = document.createElement('img');

const AD_FLAGS = ["ads", "google_ads"];
...
function getAds() {
    const ads = document.querySelectorAll("[id]");
    for (let i = 0; i < ads.length; i++) {
        for(let j = 0; j < AD_FLAGS.length; j++) {
            if (ads[i].id.indexOf(AD_FLAGS[j]) > -1) {
                ads[i].parentNode.removeChild(ads[i]);
                ads[i].parentNode.appendChild(mwElem);
            }
        }
    }
}
